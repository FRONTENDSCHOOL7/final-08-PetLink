...
export default function Market() {
    ...
    try {
      ...
      if (res.data.data > 0) {
        ...
        const productsWithCategory = productsData.map((product) => {
          const parsedItemName = JSON.parse(product.itemName); // JSON 파싱
          const category = determineCategory(parsedItemName.category);
          return {
            ...product,
            itemName: parsedItemName.itemName,
            category,
          };
        });
  ...
  const determineCategory = (categoryString) => {
        ...
        <nav>
          <NavMenu>
            {navItems.map((navItem) => (
              <li key={navItem}>
                <BtnNav
                  className={activeBtn === navItem ? 'active' : ''}
                  onClick={() => handleButtonClick(navItem)}
                >
                  {navItem}
                </BtnNav>
              </li>
